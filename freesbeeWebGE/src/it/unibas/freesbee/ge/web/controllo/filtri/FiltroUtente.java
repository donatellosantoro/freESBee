package it.unibas.freesbee.ge.web.controllo.filtri;
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        if (!servletPath.startsWith("/a4j")
         ...
         && !servletPath.equals("/schermoErrore.faces")) {
            ...
            if (utente == null) {
                if (logger.isInfoEnabled()) logger.debug("utente null");
                schermoErrore(request, response);
                return;
            }
            if (!utente.isAutenticato()) {
                if (logger.isInfoEnabled()) logger.debug("utente non autenticato");
                schermoErrore(request, response);
                return;
            }
        }
