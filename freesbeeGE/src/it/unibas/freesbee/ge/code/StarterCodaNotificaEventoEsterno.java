...
public class StarterCodaNotificaEventoEsterno extends RouteBuilder {

    private Log logger = LogFactory.getLog(this.getClass());

    public void configure() throws Exception {
        logger.info("Avvio della coda notifica esterna");
ActiveMQConnectionFactory connectionFactory = new ActiveMQConnectionFactory("vm://" + ConfigurazioneStatico.getInstance().getWebservicesIndirizzo()+"?broker.useShutdownHook=false");
        ...
        Queue destination = session.createQueue("notificaesterna");
        // Creazione del producer
        MessageProducer producer = session.createProducer(destination);
        producer.setDeliveryMode(DeliveryMode.PERSISTENT);
        // Creazione del consumer
        MessageConsumer consumer = session.createConsumer(destination);
        consumer.setMessageListener(new CodaNotificaEventoEsterno());
    }
}
