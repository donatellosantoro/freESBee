...
public class Eliminazione45 extends ContextTestSupport {

    protected Log logger = LogFactory.getLog(this.getClass());
    protected MockEndpoint resultEndpoint;

    public void testEliminazione45() throws Exception {
        try {
            logger.info("TEST - 45");
            logger.info("Sottoscrittore esistente - Categoria eventi esterna esistente non confermata attiva - Filtro Pubblicatore pubblicatore confermato per altre categorie ");
    ...
    public void testEliminazione45Verifica() throws Exception {
        try {
            logger.info("TEST - 45");
            ...
            PubblicatoreEsterno pubblicatore = daoPubblicatoreEsterno.findByTipoNome(DAOMock.TIPO_SOGGETTO_SPC, DAOMock.NOME_SOGGETTO_K);
            assertNotNull(pubblicatore);
            CategoriaEventiEsterna categoriaEventiEsterna = DAOCategoriaEventiEsternaFacade.verificaEsistenzaCategoriaEventiEsternaAttavia(DAOMock.CATEGORIA_EVENTI_ESTERNA_NON_CONFERMATA_ATTIVA_8);
            DAOPubblicatoreEsternoFacade.verificaEsistenzaPubblicatoreEsterno(categoriaEventiEsterna, pubblicatore);
