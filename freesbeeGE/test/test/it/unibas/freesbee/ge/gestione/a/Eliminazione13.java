...
public class Eliminazione13 extends ContextTestSupport {
    ...
    public void testEliminazione13() throws Exception {
        try {
            ...
            logger.info("TEST - 13");
            logger.info("Pubblicatore non esistente - Categoira eventi interna esistente attiva");
    ...
    protected RouteBuilder createRouteBuilder() {
        return new RouteBuilder() {
            ...
            public void configure() {
                from("direct:start").process(new ProcessorHibernateBegin()).process(new ProcessorSOAPReader()).process(new ProcessorGestionePubblicatori(CategoriaEventiInterna.GE_CONTROL_PROTOCOL, new DAOPubblicatoreInternoHibernate(), new DAOCategoriaEventiInternaHibernate(), new DAOSottoscrizioneInternaHibernate(), new DAOEventoPubblicatoInternoHibernate())).process(new ProcessorHibernateCommit()).to("mock:result");
            }
        };
