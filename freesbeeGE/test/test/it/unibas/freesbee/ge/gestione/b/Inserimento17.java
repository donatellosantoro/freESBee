...
public class Inserimento17 extends ContextTestSupport {
    ...
    public void testInserimento17() throws Exception {
        try {
            logger.info("TEST - 17");
            logger.info("Sottoscrittore esistente - Categoria eventi non esistente");
            ...
            sms += "<sottoscrittore><descrizione/><nome>" + DAOMock.NOME_SOGGETTO_I + "</nome><tipo>" + DAOMock.TIPO_SOGGETTO_SPC + "</tipo></sottoscrittore>";
    ...
    protected RouteBuilder createRouteBuilder() {
        return new RouteBuilder() {
            ...
            public void configure() {
                from("direct:start").process(new ProcessorHibernateBegin()).process(new ProcessorSOAPReader()).process(new ProcessorGestioneSottoscrizioni(DAOMock.CATEGORIA_EVENTI_NON_ESISTENTE_27, new DAOCategoriaEventiInternaHibernate(), new DAOCategoriaEventiEsternaHibernate(), new DAOSottoscrittoreHibernate(), new DAOPubblicatoreInternoHibernate(), new DAOPubblicatoreEsternoHibernate(), new DAOGestoreEventiHibernate())).process(new ProcessorHibernateCommit()).to("mock:result");
            }
        };
