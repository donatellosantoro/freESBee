...
public class Eliminazione38 extends ContextTestSupport {

    protected Log logger = LogFactory.getLog(this.getClass());
    protected MockEndpoint resultEndpoint;

    public void testEliminazione38() throws Exception {
        try {
            logger.info("TEST - 38");
            logger.info("Sottoscrittore esistente - Categoria eventi esterna esistente confermata attiva -Filtro Pubblicatore pubblicatore confermato e unico ");
    ...
    public void testEliminazione38Verifica() throws Exception {
        try {
            logger.info("TEST - 38");
            logger.info("Verifica eliminazione sottoscrizione");

            DAOUtilHibernate.beginTransaction();
            CategoriaEventiEsterna categoriaEventiEsterna = DAOCategoriaEventiEsternaFacade.verificaEsistenzaCategoriaEventiEsternaAttavia(DAOMock.CATEGORIA_EVENTI_ESTERNA_CONFERMATA_ATTIVA_10);
            ...
            PubblicatoreEsterno pubblicatore = daoPubblicatoreEsterno.findByTipoNome(DAOMock.TIPO_SOGGETTO_SPC, DAOMock.NOME_SOGGETTO_X);
