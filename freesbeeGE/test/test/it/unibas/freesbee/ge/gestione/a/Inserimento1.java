...
public class Inserimento1 extends ContextTestSupport {
    ...
    public void testInserimento1() throws Exception {
        try {
            ...
            logger.info("TEST - 1");
            logger.info("Pubblicatore non esistente - Categoria eventi interna non esistente");
    ...
    protected RouteBuilder createRouteBuilder() {
        return new RouteBuilder() {
            ...
            public void configure() {
                from("direct:start").process(new ProcessorHibernateBegin()).process(new ProcessorSOAPReader()).process(new ProcessorGestionePubblicatori(DAOMock.CATEGORIA_EVENTI_NON_ESISTENTE_27, new DAOPubblicatoreInternoHibernate(), new DAOCategoriaEventiInternaHibernate(), new DAOSottoscrizioneInternaHibernate(), new DAOEventoPubblicatoInternoHibernate())).process(new ProcessorHibernateCommit()).to("mock:result");
            }
        };
