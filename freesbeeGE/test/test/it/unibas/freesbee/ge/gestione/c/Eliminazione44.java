...
public class Eliminazione44 extends ContextTestSupport {

    protected Log logger = LogFactory.getLog(this.getClass());
    protected MockEndpoint resultEndpoint;

    public void testEliminazione44() throws Exception {
        try {
            logger.info("TEST - 44");
            logger.info("Sottoscrittore esistente - Categoria eventi esterna esistente non confermata attiva - Filtro Pubblicatore pubblicatore non confermato per nessuna categoria e unico ");
            ...
            sms += "<sottoscrittore><descrizione/><nome>" + DAOMock.NOME_SOGGETTO_V + "</nome><tipo>" + DAOMock.TIPO_SOGGETTO_SPC + "</tipo></sottoscrittore>";
    ...
    public void testEliminazione44Verifica() throws Exception {
        try {
            logger.info("TEST - 44");
            ...
            DAOCategoriaEventiEsternaFacade.verificaEsistenzaCategoriaEventiEsternaAttavia(DAOMock.CATEGORIA_EVENTI_ESTERNA_NON_CONFERMATA_ATTIVA_8);
            IDAOSottoscrittore daoSottoscrittore = new DAOSottoscrittoreHibernate();
            Sottoscrittore sottoscrittore = daoSottoscrittore.findByTipoNome(DAOMock.TIPO_SOGGETTO_SPC, DAOMock.NOME_SOGGETTO_V);
            assertNull(sottoscrittore);
            IDAOPubblicatoreEsterno daoPubblicatoreEsterno = new DAOPubblicatoreEsternoHibernate();
            PubblicatoreEsterno pubblicatore = daoPubblicatoreEsterno.findByTipoNome(DAOMock.TIPO_SOGGETTO_SPC, DAOMock.NOME_SOGGETTO_J);
