...
public class Inserimento8 extends ContextTestSupport {
    ...
    public void testInserimento8() throws Exception {
        try {
            logger.info("TEST - 8");
            logger.info("Sottoscrittore non esistente - Categoria eventi esterna esistente non confermata attiva - Filtro Pubblicatore Soggeto  pubblicatore confermato per altre categorie eventi ma non confermato per la categoria eventi");
            ...
            sms += "<listaFiltroPublicatore><pubblicatore><descrizione/><nome>" + DAOMock.NOME_SOGGETTO_T + "</nome><tipo>" + DAOMock.TIPO_SOGGETTO_SPC + "</tipo></pubblicatore></listaFiltroPublicatore>";
    ...
    public void testInserimento8Verifica() throws Exception {
        try {
            logger.info("TEST - 8");
            logger.info("Verifica inserimento sottoscrizione");

            DAOUtilHibernate.beginTransaction();
            CategoriaEventiEsterna catgoriaEventiEsterna7 = DAOCategoriaEventiEsternaFacade.verificaEsistenzaCategoriaEventiEsternaAttavia(DAOMock.CATEGORIA_EVENTI_ESTERNA_CONFERMATA_ATTIVA_7);
            CategoriaEventiEsterna catgoriaEventiEsterna9 = DAOCategoriaEventiEsternaFacade.verificaEsistenzaCategoriaEventiEsternaAttavia(DAOMock.CATEGORIA_EVENTI_ESTERNA_NON_CONFERMATA_ATTIVA_6);
            ...
            DAOSottoscrizioneEsternaFacade.verificaEsistenzaSottoscrittoreSottoscrizioneEsterna(catgoriaEventiEsterna9, sottoscrittore);

            IDAOCategoriaEventiEsterna daoCategoriaEventiEsterna = new DAOCategoriaEventiEsternaHibernate();

            SottoscrizioneEsterna sottoscrizione = daoCategoriaEventiEsterna.findByCategoriaEventiSottoscrittore(catgoriaEventiEsterna9, sottoscrittore);
            ...
            PubblicatoreEsterno soggettoPubblicatore = daoPubblicatoreEsterno.findByTipoNome(DAOMock.TIPO_SOGGETTO_SPC, DAOMock.NOME_SOGGETTO_T);
            assertFalse(DAOPubblicatoreEsternoFacade.isPubblicatoreEsternoPerCategoriaEventiEsterna(catgoriaEventiEsterna9, soggettoPubblicatore));
            assertTrue(DAOPubblicatoreEsternoFacade.isPubblicatoreEsternoPerCategoriaEventiEsterna(catgoriaEventiEsterna7, soggettoPubblicatore));
            ...
            assertTrue(DAOPubblicatoreEsternoFacade.isInAttesaPubblicatoreEsternoPerCategoriaEventi(catgoriaEventiEsterna9, pubblicatore));
    ...
    public void testInserimento8VerificaComunicazioneEsterna() throws Exception {
        try {
            ...
            URL url = this.getClass().getResource("/dati/e/test8Esterno.xml");
