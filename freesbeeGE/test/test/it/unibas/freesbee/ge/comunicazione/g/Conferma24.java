...
import test.it.unibas.freesbee.ge.comunicazione.f.*;
...
public class Conferma24 extends ContextTestSupport {

    protected Log logger = LogFactory.getLog(this.getClass());
    protected MockEndpoint resultEndpoint;

    public void testConferma24Prima() throws Exception {
        try {
            logger.info("TEST - 24");
            logger.info("Prima");
            ...
            sms += "<listaFiltroPublicatore><pubblicatore><descrizione/><nome>" + DAOMock.NOME_SOGGETTO_K + "</nome><tipo>" + DAOMock.TIPO_SOGGETTO_SPC + "</tipo></pubblicatore></listaFiltroPublicatore>";
            ...
            sms += "<filtroData><dataFine/><dataInizio>" + FiltroData.convertiDateToXmlDate(date.getTime()) + "</dataInizio></filtroData>";
            sms += "<listaFiltroPublicatore><pubblicatore><descrizione/><nome>" + DAOMock.NOME_SOGGETTO_K + "</nome><tipo>" + DAOMock.TIPO_SOGGETTO_SPC + "</tipo></pubblicatore>";
            sms += "<pubblicatore><descrizione/><nome>" + DAOMock.NOME_SOGGETTO_J + "</nome><tipo>" + DAOMock.TIPO_SOGGETTO_SPC + "</tipo></pubblicatore>" + "</listaFiltroPublicatore>";
    ...
    public void testConferma24VerificaPrima() throws Exception {
        try {
            logger.info("TEST - 24");
            ...
            CategoriaEventiEsterna catgoriaEventiEsterna = DAOCategoriaEventiEsternaFacade.verificaEsistenzaCategoriaEventiEsternaAttavia(DAOMock.CATEGORIA_EVENTI_ESTERNA_CONFERMATA_ATTIVA_7);
            ...
            assertFalse(DAOSottoscrizioneEsternaFacade.isInAttesaConfermaCategoriaEventi(sottoscrizione));
            assertTrue(DAOSottoscrizioneEsternaFacade.isInAttesaConfermaPubblicatori(sottoscrizione));
            assertEquals(2, sottoscrizione.getListaFiltroPublicatore().size());
            assertNotNull(sottoscrizione.getScadenzaAttesa());
            ...
            PubblicatoreEsterno pubblicatoreEsterno = daoPubblicatoreEsterno.findByTipoNome(DAOMock.TIPO_SOGGETTO_SPC, DAOMock.NOME_SOGGETTO_K);
            assertNotNull(pubblicatoreEsterno);
            assertFalse(DAOPubblicatoreEsternoFacade.isPubblicatoreEsternoPerCategoriaEventiEsterna(catgoriaEventiEsterna, pubblicatoreEsterno));
            pubblicatoreEsterno = daoPubblicatoreEsterno.findByTipoNome(DAOMock.TIPO_SOGGETTO_SPC, DAOMock.NOME_SOGGETTO_J);
            assertNotNull(pubblicatoreEsterno);
            assertFalse(DAOPubblicatoreEsternoFacade.isPubblicatoreEsternoPerCategoriaEventiEsterna(catgoriaEventiEsterna, pubblicatoreEsterno));
    ...
    public void testConferma24() throws Exception {
        logger.info("TEST - 24");
        logger.info("Si è Ricevuta Confemra Pubblicatori per la cateogria di eventi  confermata");
        try {
            //Carico il messaggio che verebbe generato dal GE
            URL url = this.getClass().getResource("/dati/g/test24.xml");
    ...
    public void testConferma24VerificaDopo() throws Exception {
        try {
            logger.info("TEST - 24");
            ...
            PubblicatoreEsterno pubblicatoreEsterno = daoPubblicatoreEsterno.findByTipoNome(DAOMock.TIPO_SOGGETTO_SPC, DAOMock.NOME_SOGGETTO_K);
            assertNotNull(pubblicatoreEsterno);
            assertTrue(DAOPubblicatoreEsternoFacade.isPubblicatoreEsternoPerCategoriaEventiEsterna(catgoriaEventiEsterna, pubblicatoreEsterno));
            pubblicatoreEsterno = daoPubblicatoreEsterno.findByTipoNome(DAOMock.TIPO_SOGGETTO_SPC, DAOMock.NOME_SOGGETTO_J);
            assertNotNull(pubblicatoreEsterno);
            assertTrue(DAOPubblicatoreEsternoFacade.isPubblicatoreEsternoPerCategoriaEventiEsterna(catgoriaEventiEsterna, pubblicatoreEsterno));
    ...
    protected RouteBuilder createRouteBuilder() {
        return new RouteBuilder() {
            ...
            public void configure() {
                from("direct:start").process(new ProcessorHibernateBegin()).process(new ProcessorSOAPReader()).process(new ProcessorGestioneSottoscrizioni(DAOMock.CATEGORIA_EVENTI_ESTERNA_CONFERMATA_ATTIVA_7, new DAOCategoriaEventiInternaHibernate(), new DAOCategoriaEventiEsternaHibernate(), new DAOSottoscrittoreHibernate(), new DAOPubblicatoreInternoHibernate(), new DAOPubblicatoreEsternoHibernate(), new DAOGestoreEventiHibernate())).process(new ProcessorHibernateCommit()).process(new ProcessorHibernateBegin()).process(new ProcessorInviaRichiestaInformazioniPubblicatori()).process(new ProcessorHibernateCommit()).to("mock:result");
            }
        };
