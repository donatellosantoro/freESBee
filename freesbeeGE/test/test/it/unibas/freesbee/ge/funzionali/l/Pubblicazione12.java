...
public class Pubblicazione12 extends ContextTestSupport {

    protected Log logger = LogFactory.getLog(this.getClass());
    protected MockEndpoint resultEndpoint;

    public void testPubblicazione12Init() throws Exception {
        logger.info("TEST - 12");
        logger.info("Init");
 ...
 Thread.currentThread().sleep(5000);
    ...
    public void testPubblicazione12Prima() throws Exception {
        try {
            logger.info("TEST - 12");
            ...
            sms += "<pubblicatore><descrizione/><nome>" + DAOMock.NOME_SOGGETTO_B + "</nome><tipo>" + DAOMock.TIPO_SOGGETTO_SPC + "</tipo></pubblicatore></listaFiltroPublicatore>";
  ...
  DAOUtilHibernate.beginTransaction();
            ...
            PubblicatoreEsterno pubblicatore2 = daoPubblicatoreEsterno.findByTipoNome(DAOMock.TIPO_SOGGETTO_SPC, DAOMock.NOME_SOGGETTO_B);
            categoria.getListaPubblicatori().add(pubblicatore1);
            categoria.getListaPubblicatori().add(pubblicatore2);
            pubblicatore1.getListaCatgoriaEventi().add(categoria);
            pubblicatore2.getListaCatgoriaEventi().add(categoria);
            daoPubblicatoreEsterno.makePersistent(pubblicatore1);
            daoPubblicatoreEsterno.makePersistent(pubblicatore2);
    ...
    public void testPubblicazione12() throws Exception {
        logger.info("TEST - 12");
        logger.info("Pubblicazione di GE (B) per la categoria Test7");
        try {
            Thread.currentThread().sleep(15000);
            //Carico il messaggio che verebbe generato dal GE
            URL url = this.getClass().getResource("/dati/l/test5.xml");
    ...
    public void testPubblicazione12VerificaConsegnaComunicazione() {
        try {
            logger.info("TEST - 12");
            ...
            URL url = this.getClass().getResource("/dati/l/test5Consegna.xml");
    ...
    public void testPubblicazione12Clean() throws Exception {
        try {
            logger.info("TEST - 12");
            logger.info("Clean");
