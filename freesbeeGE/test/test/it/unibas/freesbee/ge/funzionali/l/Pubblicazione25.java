...
public class Pubblicazione25 extends ContextTestSupport {

    protected Log logger = LogFactory.getLog(this.getClass());
    protected MockEndpoint resultEndpoint;
    protected static String id;

    public void testPubblicazione25Init() throws Exception {
        logger.info("TEST - 25");
        logger.info("Init");
    ...
    public void testPubblicazione25Prima() throws Exception {
        try {
            logger.info("TEST - 25");
    ...
    public void testPubblicazione25() throws Exception {
        logger.info("TEST - 25");
    ...
    public void testPubblicazione25VerificaNotificaComunicazione() throws Exception {
        try {
            logger.info("TEST - 25");
...
//        s = "<SOAP-ENV:Envelope xmlns:SOAP-ENV=\"http://schemas.xmlsoap.org/soap/envelope/\"><SOAP-ENV:Header/><SOAP-ENV:Body><ns2:notificaEventoPubblicato xmlns:ns2=\"http://ge.freesbee.unibas.it/\"><idEventoPubblicato>32771</idEventoPubblicato></ns2:notificaEventoPubblicato></SOAP-ENV:Body></SOAP-ENV:Envelope>";
    ...
    public void testPubblicazione25PrelevaMessaggio() throws Exception {
        logger.info("TEST - 25");
        ...
        try {
            ...
            ConsegnaEventoPubblicato risposta = port.prelevaMessaggio(id, DAOMock.NOME_SOGGETTO_D, DAOMock.TIPO_SOGGETTO_SPC, DAOMock.CATEGORIA_EVENTI_ESTERNA_CONFERMATA_ATTIVA_7);

            logger.info(risposta.getMessaggioSoap());

            URL url = this.getClass().getResource("/dati/l/test4ConsegnaNotifica.xml");
            Document doc = XmlJDomUtil.caricaXML(url.getFile(), false);
            String m = XmlJDomUtil.convertiDocumentToString(doc);

            logger.info("Comunicazione inviata: ");
            logger.info(XmlJDomUtil.formattaXML((String) risposta.getMessaggioSoap()));
            logger.info("Comunicazione di test:");
            logger.info(XmlJDomUtil.formattaXML(m));


            assertEquals(m, XmlJDomUtil.formattaXML((String) risposta.getMessaggioSoap()));

        } catch (Exception ex) {
            logger.error(ex.getMessage());
            Assert.fail("Il prelievo del messaggio ha lanciato eccezione");
        }
    }

    public void testPubblicazione25Clean() throws Exception {
        try {
            logger.info("TEST - 25");
            logger.info("Clean");
            Thread.currentThread().sleep(15000);
