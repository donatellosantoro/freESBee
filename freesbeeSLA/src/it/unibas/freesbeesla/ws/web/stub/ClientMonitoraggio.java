...
public class ClientMonitoraggio {

    private Log logger = LogFactory.getLog(this.getClass());
    private String connettoreSeriviMonitoraggio; //Indirizzo effettivo a cui inviare la richiesta
    private WSRegistroServiziImplService service;
    private IWSRegistroServizi port;
    private String indirizzoRichiesta = null;

    public ClientMonitoraggio(String connettore) throws Exception {
        this.connettoreSeriviMonitoraggio = connettore;
        ConfigurazioneStatico conf = ConfigurazioneStatico.getInstance();
        String indirizzoWSDL = connettoreSeriviMonitoraggio;
        ...
        if (!conf.getTipoNica().equalsIgnoreCase(ConfigurazioneStatico.NON_VERIFICATO)) {
            ...
            if (conf.getTipoNica().equalsIgnoreCase(ConfigurazioneStatico.NICA)) {
                ...
                indirizzoRichiesta = connettoreSeriviMonitoraggio; //E GLI DEVO INVIARE L'INDIRIZZO DEL REGISTRO
