...
public class CredentialUtil {
    ...
    public static Signature createsignatureJKS(String jksName, String password, String certificateName) throws Exception {
        Credential signingCredential = getSigningCredentialJKS(jksName, password, certificateName);
        Signature signature = (Signature) Configuration.getBuilderFactory().getBuilder(Signature.DEFAULT_ELEMENT_NAME).buildObject(Signature.DEFAULT_ELEMENT_NAME);
        signature.setSigningCredential(signingCredential);
        signature.setSignatureAlgorithm(SignatureConstants.ALGO_ID_SIGNATURE_RSA_SHA1);
        signature.setCanonicalizationAlgorithm(SignatureConstants.ALGO_ID_C14N_EXCL_OMIT_COMMENTS);
    ...
    private static KeyInfo getKeyInfo(Credential signingCredential) throws Exception {
        for (String keyName : signingCredential.getKeyNames()) {
        }
    ...
    public static Credential getSigningCredentialJKS(String jksName, String password, String certificateName) throws Exception {
        KeyStore ks = KeyStore.getInstance("JKS");
        ks.load(new FileInputStream(jksName), password.toCharArray());
        KeyStoreX509CredentialAdapter ksca = new KeyStoreX509CredentialAdapter(ks, certificateName, password.toCharArray());
...
//        InputStream isJKS = TestSAMLResponse.class.getResourceAsStream(jksName);
        if (logger.isDebugEnabled()) {
            logger.debug("Certificato: " + jksName);
        }
        InputStream isJKS = new FileInputStream(jksName);
        KeyStore keyStore = KeyStore.getInstance("JKS", "SUN");
        if (logger.isDebugEnabled()) {
            logger.debug("Carico il certificato con la passw: " + password);
        }
        keyStore.load(isJKS, password.toCharArray());
        if (logger.isDebugEnabled()) {
            logger.debug("Carico la private key con il certificateName: " + certificateName);
        }
        ...
        if (cert != null) {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("setPublicKey: " + cert.getPublicKey());
            }
        }
        return credential;
    }
