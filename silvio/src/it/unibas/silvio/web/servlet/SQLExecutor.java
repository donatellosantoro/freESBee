...
public class SQLExecutor extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        logger.info("Eseguo la select: " + query);
        String results = "";
        try {
            ResultSet rs = rsUtil.queryToRs(datiQuery, query);
            results = rsToString(rs);
        } catch (Exception ex) {
            ex.printStackTrace();
            logger.error("Impossibile eseguire la query " + query + ". Errore" + ex);
            throw new ServletException("Impossibile eseguire la query " + query + ". Errore" + ex);
        } finally {
